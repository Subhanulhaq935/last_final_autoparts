...
interface POSRegisterProps {
  ...
  onClearCart: () => void;
  onCheckout: (discountAmount: number, customerName: string, paymentMethod: "cash" | "card") => void;
}
...
export default function POSRegister({
  ...
  const handleCheckoutClick = () => {
    ...
    onCheckout(discountAmount, customerName.trim(), paymentMethod);
    setDiscountInput("");
    setCustomerName("");
          ...
          <div className="space-y-3 border-t border-slate-100 bg-white px-5 py-4 dark:border-zinc-700/60 dark:bg-zinc-900">
            ...
            {cart.length > 0 && (
              <div className="space-y-2.5 rounded-xl bg-slate-50 p-3 dark:bg-zinc-800/50">
                ...
                {/* Payment Method */}
                <div>
                  <p className="mb-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-zinc-500">
                    Payment Method
                  </p>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => setPaymentMethod("cash")}
                      className={`flex items-center justify-center gap-2 rounded-xl border-2 py-2 text-sm font-black transition-all ${
                        paymentMethod === "cash"
                          ? "border-emerald-500 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400"
                          : "border-slate-200 bg-white text-slate-500 hover:border-slate-300 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-400"
                      }`}
                    >
      ...
      <div className="space-y-3 border border-slate-200 bg-white px-5 py-4 pb-safe dark:border-zinc-700/60 dark:bg-zinc-900">
        ...
        {cart.length > 0 && (
          <div className="space-y-2.5 rounded-xl bg-slate-50 p-3 dark:bg-zinc-800/50">
            ...
            {/* Payment Method */}
            <div>
              <p className="mb-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-zinc-500">Payment Method</p>
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => setPaymentMethod("cash")}
                  className={`flex items-center justify-center gap-2 rounded-xl border-2 py-2 text-sm font-black transition-all ${
                    paymentMethod === "cash"
                      ? "border-emerald-500 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400"
                      : "border-slate-200 bg-white text-slate-500 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-400"
                  }`}
                >
                  <svg className="h-4 w-4 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 0 0-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 0 1-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 0 0 3 15h-.75M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm3 0h.008v.008H18V10.5Zm-12 0h.008v.008H6V10.5Z" />
